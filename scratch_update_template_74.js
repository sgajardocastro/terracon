...
async function run() {
  ...
  try {
    ...
    const newBodySeed = JSON.parse(fs.readFileSync('/home/nodeadmin/proyectos/lean-services-qa/body_seed_74_transformed.json', 'utf8'));

    // 1. Update tsrv_templates body_seed
    console.log('Updating tsrv_templates table...');
    await client.query('UPDATE sch_leansurvey_qa.tsrv_templates SET body_seed = $1 WHERE id_template = 74', [newBodySeed]);
    console.log('Template 74 body_seed updated in database.');

    // 2. Delete the survey 4098
    console.log('Deleting survey 4098...');
    await client.query('DELETE FROM sch_leansurvey_qa.tsrv_survey WHERE id_survey = 4098');
    console.log('Survey 4098 deleted.');

    // 3. Create a new survey for template 74
