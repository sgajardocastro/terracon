...
async function main() {
  ...
  try {
    ...
    // FETCH SEEDS FOR 146 CORRECTLY
    const resTmpl = await client.query('SELECT id_template, id_flow_tmpl, body_seed, header_seed, approval_seed, id_tipo_srv FROM sch_leansurvey_qa.tsrv_templates WHERE id_template = 146');
    if (resTmpl.rows.length === 0) {
      throw new Error('Template 146 not found');
    }
