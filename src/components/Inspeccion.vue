  ...
  <v-container style="padding-bottom: 80px;">
    <v-row dense>
      ...
      >
                ...
                >
                  ...
                  <div v-else-if="attr.type === 'decimal'">
                    <v-text-field
                      ...
                      class="bg-white"
                      inputmode="decimal"
                      :step="decimalStep(attr)"
                      :class="attr.nullable === false && !attr.default ? 'text-custom-red' : ''"
                      @keypress="onlyDecimalKeypress"
                      @blur="() => normalizeDecimalAttr(attr)"
        ...
        >
          <v-expansion-panel
            v-for="(grupo, gIdx) in getCuadrillasPPD(survey)"
                ...
                >
                  ...
                  <v-expansion-panel-text>
                    ...
                    >
                      ...
                      <!-- Horas asignadas -->
                      <v-col
                        v-if="(tarea.assigned_personnel || []).length"
                        cols="12"
                        class="mt-2"
                      >
...
const getSegmentos = (survey) => {
  ...
  try {
    ...
    /*if (!segmentos.some(s => s.label === 'GENERAL')) {
      segmentos.push({
        touch: true,
        label: "GENERAL",
        collapsible: false,
        attributes: [
          {
            ...
            // ✅ Combo con TODOS los elementos del Excel
            itemsCatalogo: [
              { id: 1, nombre: "Estación de Emergencia" },
              { id: 2, nombre: "Extintores" },
              { id: 3, nombre: "Gases Comprimidos" },
              { id: 4, nombre: "Excavación" },
              { id: 5, nombre: "Almacén Combustibles" },
              { id: 6, nombre: "Señalización" },
              { id: 7, nombre: "Baños y Duchas" },
              { id: 8, nombre: "Arnés de Seguridad" },
              { id: 9, nombre: "Comedores y Vestidores" },
              { id: 10, nombre: "Dispensador de agua" },
              { id: 11, nombre: "Oficina" },
              { id: 12, nombre: "Trabajo en altura" },
              { id: 13, nombre: "Elementos de Izaje" }
            ],

            // ✅ Según la imagen
            condicionesOptions: ["B", "M", "N/A"],

            compression: 0.25,
            body: [] // el componente asegura body[0]
          },
          { type:'textArea', label:'Condición Subestandar:', default:'' },
          { type:'textArea', label:'Medidas Correctivas:', default:'' }
        ]
      })
    }

    if (!segmentos.some(s => s.label === 'EQUIPOS')) {
      segmentos.push({
        touch: true,
        label: "EQUIPOS",
        collapsible: false,
        attributes: [
          {
            ...
            // ✅ Combo con TODOS los elementos del Excel (imagen 2)
            itemsCatalogo: [
              { id: 1, nombre: "Generador" },
              { id: 2, nombre: "Pluma Minor 300" },
              { id: 3, nombre: "Minicargador" },
              { id: 4, nombre: "Excavadora" },
              { id: 5, nombre: "Retroexcavadora" },
              { id: 6, nombre: "Motoniveladora" },
              { id: 7, nombre: "Grúa Horquilla" },
              { id: 8, nombre: "Equipos Iluminación" },
              { id: 9, nombre: "Rodillo compactador" }
            ],
    ...
    if (!segmentos.some(s => s.label === 'HERRAMIENTAS ELÉCTRICAS')) {
      segmentos.push({
        touch: true,
        ...
        attributes: [
          {
            ...
            // ✅ Combo con TODOS los elementos del Excel (imagen herramientas)
            itemsCatalogo: [
              { id: 1, nombre: "Esmeril 4 1/2\"" },
              { id: 2, nombre: "Serrucho eléctrico" },
              { id: 3, nombre: "Esmeril 7\"" },
              { id: 4, nombre: "Rotomartillo" },
              { id: 5, nombre: "Soldadora" },
              { id: 6, nombre: "B. Sumergible" },
              { id: 7, nombre: "Extensiones" },
              { id: 8, nombre: "Tablero eléctrico" },
              { id: 9, nombre: "Demoledor" },
              { id: 10, nombre: "Máquina Inyectora" },
              { id: 11, nombre: "Curvadora" },
              { id: 12, nombre: "Motosierra" },
              { id: 13, nombre: "Vibropison" },
              { id: 14, nombre: "Desbrozadora" },
              { id: 15, nombre: "Barreno Ahoyador" },
              { id: 16, nombre: "Código de color del mes" }
            ],
...
function normalizeBodyExec(bodyExec) {
  // Si viene como string JSON, úsalo tal cual (sin doble stringify)
  if (typeof bodyExec === 'string') {
    const t = bodyExec.trim()
    return t === '' ? 'null' : t
  }

  // Si viene como objeto/array, stringify normal
  return JSON.stringify(sanitizePPDBodyExec(bodyExec) ?? null)
}
...
async function putSurvey (id_survey) {
  ...
  const bodyLimpio = sanitizePPDBodyExec(
    stripBase64FromBodyExec(surveyDetailStore.surveyDetail[0].body_exec)
  )
  ...
  const payload = {
    ...
    body_exec: JSON.stringify(bodyLimpio),
...
async function handleTerminarSurvey () {
  ...
  try {
    ...
    // 1) Guardar body_exec en estado "Ejecución" (usa cola, pero en este caso hay internet)
    const result = await putSurvey(current.id_survey)
    if (!result || result.ok === false) {
      dialogLoading.value = false
      alert('Error al guardar antes de pasar a flujo.')
      return
    }
