...
const PLAN_C_TEMPLATE_ID = 201
const TEMPLATE_PADRE_ID = 110
const TEMPLATE_HIJO_ID = 145
...
const crearSurveyPlanCDesdeFila = async (fila) => {
  const baseTemplate = resolvePlanCTemplate()
  if (!baseTemplate) {
    throw new Error(`No existe template ${PLAN_C_TEMPLATE_ID} para Plan C.`)
  }

  const header_seed = baseTemplate.header_seed || {}
  const approval_seed = baseTemplate.approval_seed || {}
  const id_flow_tmpl = baseTemplate.id_flow_tmpl || null
  const body_seed_obj = baseTemplate.body_seed_obj || { segmentos: [] }
  ...
  const asignacion = extractAsignacionPlanCFromRow(fila)
  upsertAsignacionPlanCInBodyExec(body_exec, asignacion)
  const fechaPlanFila = getFechaPlanFromRow(fila)

  const payload = {
    id_tipo_srv: 36,
    id_template: PLAN_C_TEMPLATE_ID,
...
const crearSurveysDesdeExcel = async () => {
  ...
  for (const [idx, fila] of filasTerracon.entries()) {
    ...
    try {
      const idSurvey = await crearSurveyPlanCDesdeFila(fila)
      ids.push(idSurvey)
      ...
      console.error('[ExcelPreview] Error creando survey Plan C:', fila, err?.response?.data || err)
    }
  }

  return { creados: ok, ids, fallados: fail, duplicadas, omitidasSinDatos, errores }
}
...
onMounted(async () => {
  fechaProyectada.value = hoyYYYYMMDD()
  await Promise.all([obtenerClientes(), obtenerTemplates(), obtenerUsuarios()])
...
function parseBodySeed(rawSeed) {
  const parsed = safeJsonParse(rawSeed, { segmentos: [] })
  if (parsed && Array.isArray(parsed.segmentos)) return parsed
  return { segmentos: [] }
}

function mergeBodySeeds(seedA, seedB) {
  const out = deepClone(seedA || { segmentos: [] })
  if (!Array.isArray(out.segmentos)) out.segmentos = []
  const segmentosOut = out.segmentos
  const addFrom = (seed) => {
    for (const seg of (seed?.segmentos || [])) {
      const segLabel = String(seg?.label || '').trim()
      if (!segLabel) continue
      let targetSeg = segmentosOut.find(s => String(s?.label || '').trim().toUpperCase() === segLabel.toUpperCase())
      if (!targetSeg) {
        targetSeg = deepClone(seg)
        if (!Array.isArray(targetSeg.attributes)) targetSeg.attributes = []
        segmentosOut.push(targetSeg)
        continue
      }
      if (!Array.isArray(targetSeg.attributes)) targetSeg.attributes = []
      const existing = new Set(targetSeg.attributes.map(a => `${normalizePlain(a?.label)}::${String(a?.type || '').toLowerCase()}`))
      for (const attr of (seg?.attributes || [])) {
        const key = `${normalizePlain(attr?.label)}::${String(attr?.type || '').toLowerCase()}`
        if (existing.has(key)) continue
        targetSeg.attributes.push(deepClone(attr))
        existing.add(key)
      }
    }
  }
...
function resolvePlanCTemplate() {
  const tpl201 = templatesSurvey.value.find(t => Number(t?.id_template) === PLAN_C_TEMPLATE_ID) || null
  if (!tpl201) return null

  const tpl110 = templatesSurvey.value.find(t => Number(t?.id_template) === TEMPLATE_PADRE_ID) || null
  const tpl145 = templatesSurvey.value.find(t => Number(t?.id_template) === TEMPLATE_HIJO_ID) || null

  const seed201 = parseBodySeed(tpl201?.body_seed)
  const seed110 = parseBodySeed(tpl110?.body_seed)
  const seed145 = parseBodySeed(tpl145?.body_seed)
  const hasSeed201 = Array.isArray(seed201?.segmentos) && seed201.segmentos.length > 0
  const merged110145 = mergeBodySeeds(seed110, seed145)
  const body_seed_obj = hasSeed201 ? seed201 : merged110145

  return {
    header_seed: tpl201?.header_seed || tpl110?.header_seed || {},
    approval_seed: tpl201?.approval_seed || tpl110?.approval_seed || {},
    id_flow_tmpl: tpl201?.id_flow_tmpl || tpl110?.id_flow_tmpl || null,
    body_seed_obj
  }
}
...
function resolveUserIdFromExcelValue(rawValue) {
  const val = String(rawValue || '').trim()
  if (!val) return null
  const numeric = Number(val)
  if (Number.isFinite(numeric) && numeric > 0) return numeric

  const normVal = normalizePlain(val)
  const users = usuarios.value || []
  const exact = users.find(u => normalizePlain(formatUserFullName(u)) === normVal)
  if (exact?.id_user) return Number(exact.id_user)

  const includes = users.find(u => {
    const full = normalizePlain(formatUserFullName(u))
    return full && (full.includes(normVal) || normVal.includes(full))
  })
  if (includes?.id_user) return Number(includes.id_user)

  return null
}

function resolveEmpresaIdFromExcelValue(rawValue) {
  const val = String(rawValue || '').trim()
  if (!val) return null
  const numeric = Number(val)
  if (Number.isFinite(numeric) && numeric > 0) return numeric
  ...
  const normVal = normalizePlain(val)
  const empresas = clientes.value || []
  const exact = empresas.find(e => normalizePlain(e?.name_empresa) === normVal)
  if (exact?.id_empresa) return Number(exact.id_empresa)

  const includes = empresas.find(e => {
    const emp = normalizePlain(e?.name_empresa)
    return emp && (emp.includes(normVal) || normVal.includes(emp))
  })
  if (includes?.id_empresa) return Number(includes.id_empresa)

  return null
}

function extractAsignacionPlanCFromRow(fila) {
  const empresaRaw = getCellByAnyHeader(fila, ['EMPRESA', 'CONTRATISTA', 'EMPRESA CONTRATISTA'])
  const supervisorRaw = getCellByAnyHeader(fila, ['SUP', 'SUPERVISOR', 'SUP TERRACON', 'SUPERVISOR TERRACON'])
  const tecnicoRaw = getCellByAnyHeader(fila, ['TEC', 'TECNICO', 'TÉCNICO', 'NOMBRE TECNICO', 'NOMBRE TÉCNICO'])
  const supervisorId = resolveUserIdFromExcelValue(supervisorRaw)
  const tecnicoId = resolveUserIdFromExcelValue(tecnicoRaw)
  const empresaId = resolveEmpresaIdFromExcelValue(empresaRaw)

  return {
    empresaId,
    supervisorId,
    tecnicoId,
    empresaNombre: String(empresaRaw || '').trim(),
    supervisorNombre: String(supervisorRaw || '').trim(),
    tecnicoNombre: String(tecnicoRaw || '').trim()
  }
}

function upsertAsignacionPlanCInBodyExec(bodyExec, asignacion) {
  const empresaId = Number(asignacion?.empresaId || 0) || null
  const supervisorId = Number(asignacion?.supervisorId || 0) || null
  const tecnicoId = Number(asignacion?.tecnicoId || 0) || null
  const empresaNombre = String(asignacion?.empresaNombre || '').trim()
  const supervisorNombre = String(asignacion?.supervisorNombre || '').trim()
  const tecnicoNombre = String(asignacion?.tecnicoNombre || '').trim()

  if (!bodyExec || typeof bodyExec !== 'object') return
  bodyExec.asignaciones = {
    Contratista: empresaId || 0,
    Supervisor: supervisorId || 0,
    Tecnico: tecnicoId || 0,
    Marca: 'PLAN_C',
  ...
  setAttrIfNotBlank(bodyExec, 'Empresa', empresaNombre)
  setAttrIfNotBlank(bodyExec, 'Contratista', empresaNombre)
  setAttrIfNotBlank(bodyExec, 'Supervisor', supervisorNombre)
  setAttrIfNotBlank(bodyExec, 'Sup Terracon', supervisorNombre)
  setAttrIfNotBlank(bodyExec, 'Tecnico', tecnicoNombre)
  setAttrIfNotBlank(bodyExec, 'Técnico', tecnicoNombre)

  if (!Array.isArray(bodyExec.segmentos)) bodyExec.segmentos = []
  let segmento = bodyExec.segmentos.find(s => normalizePlain(s?.label) === 'ASIGNACION PLAN C')
  if (!segmento) {
    segmento = {
      label: 'ASIGNACION PLAN C',
      touch: false,
      posicion: 997,
      collapsible: true,
      attributes: []
  ...
  const upsertAttr = (label, value, readOnly = false) => {
    const idx = segmento.attributes.findIndex(a => normalizePlain(a?.label) === normalizePlain(label))
    const attrObj = {
      type: 'textField',
      label,
    ...
    if (idx >= 0) segmento.attributes[idx] = { ...segmento.attributes[idx], ...attrObj }
    else segmento.attributes.push(attrObj)
  }
  upsertAttr('MARCA PLAN', 'PLAN_C', true)
  upsertAttr('EMPRESA PLAN_C', empresaNombre || '')
  upsertAttr('SUP PLAN_C', supervisorNombre || '')
  upsertAttr('TEC PLAN_C', tecnicoNombre || '')
}
