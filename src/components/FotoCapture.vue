  ...
  if (file && file.type.startsWith('image/')) {
    ...
    reader.onload = async (e) => {
      ...
      img.onload = async () => {
        ...
        const nuevoObjeto = {
          base64: '',
          url: '/archivo/' + dataArchivo?.archivo.name_doc_interno,       // si viene la url desde el servidor
...
async function uploadFileFromBase64(base64Data, originalName, mimeType) {
  ...
  formData.append('path_doc', '/u05/LeanDocs/')
  formData.append('id_user', 1)
  formData.append('estado', '1')

  try {
    const response = await apiAxios.post('/archivo', formData, {
      headers: {
