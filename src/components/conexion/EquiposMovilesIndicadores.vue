<template>
  <div class="mb-2">
    ...
    <div class="d-flex align-center mb-3">
      ...
      <span class="text-subtitle-2 font-weight-bold text-primary-dark uppercase-lite">Indicadores Equipos
        Móviles</span>
    </div>
...
const loadKpis = async () => {
    try {
        ...
        // Find the group related to 'DocumentacionEquiposConexion'
        // Assuming the structure is [ { grupo: [ ... ] } ]
        const groupData = data[0]?.grupo?.find(g => g.titulo === 'DocumentacionEquiposConexion');

        if (groupData && groupData.indicadores) {
            kpis.value = groupData.indicadores.map(ind => ({
                title: ind.titulo,
                value: 0,
                uri: ind.uri,
                icon: ind.icono,
            ...
            kpis.value = kpis.value.map((kpi, idx) => ({ ...kpi, value: values[idx] ?? 0 }));
        }
    } catch (error) {
        console.error('Error loading KPIs from dataConexionDocumentos.json:', error);
    }
};
